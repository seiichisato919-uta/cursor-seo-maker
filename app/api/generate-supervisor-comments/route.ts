    ...
    if (data.h2Blocks && Array.isArray(data.h2Blocks)) {
      ...
      try {
        for (const block of limitedBlocks) {
          ...
          // H3が16個ある場合など、長い記事でも処理できるように4000文字に制限
          const contentToProcess = block.writtenContent && block.writtenContent.length > 4000 
            ? block.writtenContent.substring(0, 4000) + '\n\n（...以下省略）'
            : block.writtenContent || '';
          
          console.log(`[Supervisor Comments] Block ${block.id} - Content length: ${block.writtenContent?.length || 0}, Processing: ${contentToProcess.length}`);
          ...
          // 既存の記事内容を保持したまま、監修者の吹き出しを挿入するように指示（簡潔版）
          fullPrompt += `

## 【出力形式】
1. 既存の記事内容をそのまま保持
2. 適切な箇所に監修者の吹き出しを挿入（形式: <佐藤誠一吹き出し>コメント</佐藤誠一吹き出し>）
3. 見出しや分析結果は一切出力しない
