...
export function getWritingPrompt(data: any): string {
  ...
  // 対話形式の指示を削除
  if (prompt) {
    // STEP1の対話形式の部分を削除（### **STEP1: から始まる部分を削除）
    // STEP1全体を削除（STEP2の直前まで）
    prompt = prompt.replace(/### \*\*STEP1:.*?### \*\*STEP2:/gs, '### **STEP2:');
    
    // STEP2内の対話形式の指示を削除（ユーザーとの対話を待つ部分）
    // 「執筆の基本ルール」セクションの対話部分を削除（ただし執筆ルール自体は残す）
    prompt = prompt.replace(/### \*\*執筆の基本ルール：\*\*[\s\S]*?記事全体を一度に完成させようとしてはいけません\*\*/g, '');
    // 「H2ブロック執筆の流れ」セクションを削除（対話形式のため）
    prompt = prompt.replace(/### \*\*H2ブロック執筆の流れ：\*\*[\s\S]*?すべてのH2ブロックが完成するまで、2〜4を繰り返す/g, '');
    // 「執筆完了後の確認メッセージ例」セクションを削除（対話形式のため）
    prompt = prompt.replace(/### \*\*執筆完了後の確認メッセージ例：\*\*[\s\S]*?次は【H2ブロック2】を執筆しますか？それとも他のH2ブロックを先に執筆しますか？」/g, '');
    // 「禁止事項」セクションの対話部分を削除
    prompt = prompt.replace(/\*\*禁止事項：\*\*[\s\S]*?記事全体を一度に完成させようとしてはいけません/g, '');
  }
...
export function getInternalLinkPrompt(data: { article: string; spreadsheetData?: any }): string {
  ...
  if (!basePrompt) {
    ...
    basePrompt = `あなたはSEOメディアの内部リンク最適化を専門とするコンサルタントです。
既存の記事内容に「参考記事：記事タイトル(URL)」形式の内部リンクを1つ以上挿入してください。`;
  }
  
  // プロンプトから不要なセクションを削除（タイムアウト対策）
  ...
  // 記事一覧を簡潔な形式に変換（最大20件まで）
  let articleListText = '';
  if (data.spreadsheetData && Array.isArray(data.spreadsheetData) && data.spreadsheetData.length > 0) {
    const limitedArticles = data.spreadsheetData.slice(0, 20); // 最大20件まで
    articleListText = limitedArticles.map((item: any) => `${item.title} (${item.url})`).join('\n');
  }
  
  // 記事内容が長すぎる場合は先頭6000文字に制限（タイムアウト対策）
  const articleToProcess = data.article && data.article.length > 6000 
    ? data.article.substring(0, 6000) + '\n\n（...以下省略）'
    : data.article;
  
  // 元のプロンプトファイルの重要な部分を保持しつつ、簡潔な指示を追加
  return `${basePrompt}

## 記事一覧
${articleListText || '記事一覧がありません。'}
...
export function getSalesLocationPrompt(data: any): string {
  ...
  // 記事全体で既に「※ここにセールス文を書く」が何箇所あるかを取得
  const currentTotalMarkers = data.currentTotalMarkers || 0;
  const shouldInsertMarker = data.shouldInsertMarker !== false; // デフォルトはtrue
...
記事全体で既に「※ここにセールス文を書く」が挿入されている箇所: ${currentTotalMarkers}箇所

## ⚠️【超重要】タスクの説明⚠️
あなたのタスクは以下の通りです：
1. **記事内容を読んで、セールス文を挿入すべき箇所を選ぶ（記事全体で2箇所程度を目安）**
2. **既存の記事・文章を一切改変せず、選んだ箇所に「※ここにセールス文を書く」を挿入する**
3. **記事全体で既に「※ここにセールス文を書く」が${currentTotalMarkers}箇所あります。記事全体で2箇所程度にするため、${currentTotalMarkers >= 2 ? 'このブロックには「※ここにセールス文を書く」を挿入しないでください。既存の記事内容をそのまま出力してください。' : 'このブロックに「※ここにセールス文を書く」を挿入する必要がない場合は、既存の記事内容をそのまま出力してください（必ずしも挿入する必要はありません）。'}**
