...
${articleList}`;
  ...
  const articleToProcess = data.article && data.article.length > 8000 
    ? data.article.substring(0, 8000) + '\n\n（...以下省略）'
    : data.article;
...
**⚠️絶対に守ってください：内部リンクを1つも挿入しない出力は絶対にNGです。必ず1つ以上の「参考記事：記事タイトル(URL)」形式の内部リンクを挿入してください。**
...
## ⚠️【最終警告】⚠️
**もしあなたが内部リンクを1つも挿入しない出力を返した場合、それは完全に間違っています。**
**必ず「参考記事：記事タイトル(URL)」形式の内部リンクを1つ以上挿入してください。**
**内部リンクを挿入しないことは絶対に許されません。**`;
}
