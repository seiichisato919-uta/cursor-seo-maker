...
export function getInternalLinkPrompt(data: { article: string; spreadsheetData?: any }): string {
  // プロンプトを大幅に簡素化（タイムアウト対策）
  // 元のプロンプトファイルは使わず、最小限の指示のみ
...
## 出力形式
1. 既存の記事内容をそのまま保持
2. 適切な箇所に「参考記事：記事タイトル(URL)」を1行で追加
3. 内部リンクは必ず「参考記事：」（全角コロン）で始める
4. 分析結果やメッセージは一切出力しない

## 重要
内部リンクを1つも挿入しない出力はNGです。必ず1つ以上の内部リンクを挿入してください。`;
}
